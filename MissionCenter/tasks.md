...
| LLM-T1 | 多 AI 辯論基礎流程 | Task | LLM-E1 | P1 | Done | Codex |  | 無 | 使用 ChatGPT/Gemini/Grok 實際試跑，使用者回報可運行 | 2 | mvp | 已完成 |
| LLM-T2 | Claude 參與者支援 | Task | LLM-E1 | P2 | Done | Codex |  | 無 | 使用者 Chrome 實機跑四家 AI 辯論，Claude 可參與 | 2 | provider, claude | 使用者回報四家 AI 實機辯論正常 |
...
| LLM-T7 | Chrome 實機驗收 | Task | LLM-E1 | P1 | Done | 使用者 | LLM-T3, LLM-T4 | 無 | Chrome 實際分頁中跑 ChatGPT/Gemini/Grok/Claude | 2 | manual-smoke | 使用者回報四家 AI 實機沒問題 |
...
| LLM-T14 | 作者模式羊解鎖 | Task | LLM-E1 | P2 | Done | Codex | LLM-T12 | 無 | `npm test` 53/53、store zip 內容檢查 | 1 | dev-tool, entitlement | 五連點 Free badge 依序顯示羊文字，最後解鎖 🐑；依使用者要求保留進商店 zip |
...
| LLM-T18 | Free 五連點羊模式彩蛋 | Task | LLM-E1 | P2 | Done | Codex | LLM-T17 | 無 | 單元測試與封裝內容檢查 | 1 | easter-egg, ui | 第 1 次固定台詞，第 3 次從 10 句隨機挑選，第 5 次顯示 🐑 並保留 YouTube 連結 |
| LLM-T19 | Gemini 可靠送出與確認 | Task | LLM-E1 | P1 | Done | Codex | LLM-T17 | 在登入 Gemini 的 Chrome 跑一次完整送出與收回 | 單元測試、content script 安全檢查與 Chrome 實機試跑 | 2 | gemini, automation, reliability | 自動測試已通過；按鈕未確認時只補一次 Enter，仍失敗會明確報錯 |
| LLM-E2 | 0.4.7 信任與穩定性版本 | Epic |  | P1 | Done | Codex | LLM-E1 | 完成五家 provider 登入態 smoke 後關閉週期 | 自動測試、語法檢查、封裝、Chrome 實機 smoke | 13 | hardening, privacy, meta-ai | 自動驗證與封裝完成；Meta AI 仍為 Beta |
...
| LLM-T22 | Provider adapter 與 Meta AI Beta | Task | LLM-E2 | P1 | Done | Gemini/Codex |  | 無 | provider、manifest、content safety、Chrome Meta smoke | 3 | provider, meta-ai, beta | adapter、自動測試與五家 provider 實機 smoke 完成；Meta 預設未啟用 |
...
| LLM-T25 | 0.4.7 完整驗證與封裝 | Task | LLM-E2 | P1 | Done | Codex | LLM-T20, LLM-T21, LLM-T22, LLM-T23, LLM-T24 | 無 | `npm test`、`node --check`、`git diff --check`、`npm run package` | 1 | verification, packaging | 149/149 pass；0.4.7 zip 已產生並檢查內容 |
